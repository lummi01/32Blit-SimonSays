...
void render(uint32_t time) 
{
    ...
    screen.blit(background, Rect(0, 0, 160, 120), Point(0, 0));
    ...
    if (bp.number > 0)
    {
        ...
        if (bp.number == 1)
        {
            screen.sprite(Rect(0, 0, 10, 6), Point(40, -1));
        }
        else if (bp.number == 2)
        {
            screen.sprite(Rect(0, 0, 6, 10), Point(93, 20), SpriteTransform::R90);
        }
        else if (bp.number == 3)
        {
            screen.sprite(Rect(0, 0, 10, 6), Point(40, 73), SpriteTransform::R180);
        }
        else if (bp.number == 4)
        {
            screen.sprite(Rect(0, 0, 6, 10), Point(19, 20), SpriteTransform::R270);
        }
    }

    screen.alpha = 255;
    screen.pen = Pen(255, 255, 255);
    screen.text(std::to_string(p.level) + "/" + std::to_string(best), font, Point(159, 2), true, TextAlign::top_right);

    if (state == 0)
    {
        screen.alpha = 128;
        screen.pen = Pen(128, 128, 128);        
        screen.rectangle(Rect(0, 88, 160, 12));
        screen.alpha = 255;
        screen.pen = Pen(0, 0, 0);
        screen.text("PRESS A TO START", font, Point(37, 91), true);
        screen.pen = Pen(255, 255, 255);
        screen.text("PRESS A TO START", font, Point(37, 90), true);
    }    
}
